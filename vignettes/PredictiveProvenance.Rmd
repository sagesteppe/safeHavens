...
western_states <- spData::us_states |> ## for making a quick basemap. 
  dplyr::filter(NAME %in% c('Utah', 'Arizona', 'Colorado', 'New Mexico')) |>
...
bmap + 
  geom_sf(data = eSDM_model$TrainData, aes(color = occurrence), shape = 2)+ 
  geom_sf(data = eSDM_model$TestData, aes(color = occurrence))

eSDM_model$ConfusionMatrix
```
...
### need to clip clusters sf and raster to suitable_habitat
plot(future_clusts$suitable_habitat)
plot(threshold_rasts$FinalRasters['Threshold'])

plot(future_clusts$novel_mask)
...
bmap + 
  geom_sf(data = ENVIbs$Geometry, fill = 'blue') #+ 
 # geom_sf(data = future_clusts$clusters_sf, fill = 'black')
