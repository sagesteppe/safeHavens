...
```{r}
brms::pp_check(sdModel$Model, type = "bars") 
brms::pp_check(
  sdModel$Model, 
  type = "stat", 
  stat = "mean"
...
loo_probs |>
  ...
  labs(x = "Mean LOO predicted probability", y = "Observed frequency",
       title = "LOO calibration")

# ROC per draw then summarise
pred_probs |>
  group_by(.draw) |>
  summarise(
    auc = as.numeric(
  ...
  ggplot(aes(x = auc)) +
  stat_halfeye() +
  labs(x = "AUC", title = "Posterior distribution of AUC")

roc(loo_probs$observed, loo_probs$pred_prob) |>
  ggroc() +
  geom_abline(slope = 1, intercept = 1, linetype = "dashed") +
  labs(title = "LOO ROC curve")
...
The AUC value will be optimistic
...
dharma_res <- createDHARMa(
  ...
  observedResponse  = df$y,
  fittedPredictedResponse = apply(sims, 2, mean)
)

plot(dharma_res)                        # QQ + residuals vs fitted
plotResiduals(dharma_res, df$x1)        # residuals vs a specific predictor
```


```{r plot the posteriors}
...
in the partial effects plot, each variables contribution is shown across it's entire range, when averaged over other variables. 
...
points(obby$SamplePoints, cex = 0.3)
```
...
par(mfrow = c(1, 3))
plot(as.factor(obby$SummaryRaster$consensus))
...
cus_pal <- RColorBrewer::brewer.pal(
  n = nCl, 
  name = "Set3"
  )
names(cus_pal) <- 1:nCl
...
loo_probs |>
  ...
  labs(
    ...
    subtitle = "LOO predicted probability distributions for observed presences (1) and absences (0)",
...
plots <- purrr::map(
  ...
  marginaleffects::plot_predictions(sdModel$Model, condition = .x) +
    ...
    ggplot2::update_geom_defaults("line",   list(fill = "#dce3f0"))
) 
