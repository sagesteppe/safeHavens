...
rr$BetaCoefficients[ order(rr$BetaCoefficients$Estimate), ]
```
...
```{r view top 3 ranks side by side,eval=F}
plot(obby$ConsensusRaster)
plot(obby$Rank2Raster)
plot(obby$Rank3Raster)
```
