...
A few spatial data sets that should be suitable for nearly all users IBR applications are included in `safeHavens` for this example, but the full data sets are not shipped with the package. 
They include: 
...
Each of the environmental variable data sources needs some pre-processing before being feed into the IBR workflow, most of them just need to be converted from vector to raster formats - a process we call 'rasterization'. 
We will also rescale the tri rasters, to put them on a similar numeric range as the other data sets. 
...
This is certainly the most difficult part of this workstream to tune, and often relies on subjective expert judgement.
...
A default argument to cluster IBR is the probablity surface predictions from an SDM. 
...
Note that we will have to reverse the predictions from this layer, currently higher values mean more suitable habitat. 
We want the higher values going into the probability surface function to mean more resistance to movement. 
...
Indeed something like terrain ruggedness may have only a small effect until reaching certain thresholds. 
...
We will apply a fourth order polynomial to the input tri data to accomodate a non-linear effect. 
