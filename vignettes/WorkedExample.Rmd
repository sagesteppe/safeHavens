...
spp_concave <- lapply(sppL, FUN = concavities, d = 20000, rat = 0.4)
spp_convex  <- lapply(sppL, concavities, d = 20000, rat = 1.0)
...
lapply(sppL, vario_estimate, bb_dist = 50000)

#base +
#  geom_sf(data = dat2, fill = 'red', alpha = 0.2) 
```
...
```{r perform sampling}
eas <- lapply(spp_concave, FUN = EqualAreaSample, n = 10, pts = 250, planar_projection = 5070, reps = 25)
base +
...
spp_concave <- lapply(spp_concave, FUN = function(x){ 
  st_as_sf(x) |> 
    mutate(Range = 1, .before = geometry) |> ## this `field` is rasterized
    st_transform(4326) |> 
...
ibd_samples <- lapply(
  spp_concave, 
  FUN = IBDBasedSample, 
  n = 10, fixedClusters = FALSE, template = template, planar_proj = 5070)
...
## write out the template for future use. 
