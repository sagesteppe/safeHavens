...
template <- terra::rast(terra::ext(bb), terra::crs(pts) resolution = c(0.1, 0.1))
...
ibd_samples_priority <- ibd_samples %>%
  ...
  purrr::map(~ list(PrioritizeSample(.x, reps = 99, n_breaks = 3)))
