...
Given then one of these sampling scheme may be followed for quite some time, I think it is best practice to save many of these objects. 
...
Note that while the buffered polygons make look a little nebulous now, we'll have them on maps with some more context in just a couple steps. 
...
```{r Map themes}
x_extra_buff <- sf::st_buffer(x_buff, 100000) |>
...
```{r Grid Based Samples, warning = FALSE, width="200%", message = FALSE}
grid_buff <- GridBasedSample(x_buff, planar_proj, gridDimensions = tgs) 

gbs.p <- map + 
  geom_sf(data = grid_buff, aes(fill = factor(Assigned))) + 
...
eas.p <- map + 
  geom_sf(data = eas.sf, aes(fill = factor(ID))) + 
...
os.p <- map + 
  geom_sf(data = os.sf, aes(fill = factor(ID))) + 
...
While the above sampling schemes are implicetly based around the former idea, the latter is the most essential for germplasm conservation. 
This function explicitly uses IBD to develop a sampling scheme, and does not obfusciate it with any other parameters. 
...
For some reason I am convinced this is a product many people will prefer relative to the 'post-processed' object which we will return below. 
...
sdModel <- readRDS(
  file.path(system.file(package="safeHavens"), 'data', 'sdModel.rds')
  )

sdModel$Predictors <- terra::rast(
  file.path(system.file(package="safeHavens"), 'data', 'Predictors.tif')
)
```

Once these data are loaded into R, we will rescale the rasters (using `RescaleRasters`) which will serve as surfaces to predict from (this is also done above!), then we will run the algorithm (`EnvironmentalBasedSample`). 
...
```{r echo = FALSE}
rm(ENVIbs_thresh, ENVIbs_clipped, ENVIbs.p.thresh, ENVIbs.p.clipped, p, ENVIbs, rr, ENVIbs.p.supp)
```
