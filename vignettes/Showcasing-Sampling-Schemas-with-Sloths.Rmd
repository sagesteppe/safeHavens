...
terra::writeRaster(sdModel$Predictors, '../data/Predictors.tif')
saveRDS(sdModel, '../data/sdModel.rds')
...
```{r Grid Based Samples, warning = FALSE, width="200%"}
...
```{r Point Based Sample, warning=FALSE}
...
#  geom_sf_label(data = os.sf, aes(label = ID), alpha = 0.4) + 
  ...
  labs(title = 'Opporunistic Sample / Point Based')
...
```{r Isolatation by Distance Example, warning=FALSE}
...
```{r Load data for Environmental Based Sample}
sdModel <- readRDS(
...
ENVIbs <- EnvironmentalBasedSample(
  ...
  taxon = 'Bradypus_test', 
  f_rasts = sdm, n = 20, fixedClusters = TRUE, n_pts = 500, 
  planar_proj = 
    '+proj=laea +lon_0=-421.171875 +lat_0=-16.8672134 +datum=WGS84 +units=m +no_defs',
...
map + 
  ...
  labs(title = 'Environmental Based Sample')
