...
```{r Map themes}
carto_theme <- function(){
  theme(
  legend.position = 'none', 
  panel.background = element_rect(fill = "aliceblue"), 
  panel.grid.minor.x = element_line(colour = "red", linetype = 3, size = 0.5)
  )
}
...
```{r Grid Based Samples, warning = FALSE}
...
ggplot(data = grid_buff) + 
  ...
  geom_sf_label(aes(label = Assigned), alpha = 0.4) + 
  carto_theme()  + 
...
ggplot(data = pbs.sf) + 
  ...
  geom_sf_label(aes(label = ID), alpha = 0.4) + 
  carto_theme() + 
...
```{r Isolatation by Distance Example}
...
ggplot() + 
  geom_sf(data = ebs.sf, aes(fill = n))  + 
  carto_theme() + 
  xlab('Longitude (x)') + 
  ylab('Latitude (y)') +
  labs(title = 'Sampling by Ecoregion (Largest)')
