...
assign_contested_line <- function(cluster_r, contested) {
  ...
  edge_list <- do.call(
    rbind,
    lapply(seq_along(adj), function(i) {
  ...
  for (comp_id in unique(comps)) {
    ...
    nbr_vals <- terra::values(cluster_r)[terra::adjacent(
      cluster_r,
      cells = cells,
      directions = 8
    )]
    nbr_vals <- nbr_vals[!is.na(nbr_vals)]
    if (length(nbr_vals) < 2) {
      next
    } # cannot split, just leave assigned later
...
finalize_cluster <- function(cluster_r, pop_raster, directions = 8) {
  ...
  na_cells <- which(
    is.na(terra::values(cluster_pop)) & !is.na(terra::values(pop_raster))
  )
