...
EqualAreaSample <- function(
  ...
  variance <- unlist(lapply(voronoiPolygons, function(x) {
    stats::var(as.numeric(sf::st_area(x)) / 10000)
  }))
...
VoronoiSamplerEAS <- function(x, kmeans_centers, reps, pts, n, planar_proj) {
  ...
  kmeans_centers <- stats::setNames(
    data.frame(
      kmeans_res['centers'],
      seq_len(nrow(kmeans_res['centers'][[1]]))
    ),
    # use the centers as voronoi cores ...
    c('X', 'Y', 'ID')
  )
  kmeans_centers <- sf::st_as_sf(
    kmeans_centers,
    coords = c('X', 'Y'),
    crs = planar_proj
  )
  ...
  voronoi_poly <- sf::st_intersection(
    ...
    voronoi_poly,
    sf::st_union(
      sf::st_transform(x, sf::st_crs(voronoi_poly))
    )
  ) |>
