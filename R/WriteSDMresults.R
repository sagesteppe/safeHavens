...
writeSDMresults <- function(
  ...
  if (!missing(cv_model)) {
    ...
    saveRDS(
      # all cross validation information
      cv_model,
      file = file.path(path, 'Fitting', paste0(taxon, '-Fitting.rds'))
    )
  } else {
    message('param: `cv_model` not supplied to fn, not saving it.')
  }

  if (!missing(pcnm)) {
    dir.create(file.path(path, 'PCNM'), showWarnings = FALSE)
    terra::writeRaster(
      # save the pcnm layers we created.
      pcnm,
      overwrite = TRUE,
      file = file.path(path, 'PCNM', paste0(taxon, '-PCNM.tif'))
    )
  } else {
    message('param: `pcnm` not supplied to fn, not saving it.')
  }
  ...
  if (!missing(model)) {
    ...
    message('param: `model` not supplied to fn, not saving it.')
  }
  ...
  if (!missing(coef_tab)) {
    ...
    message('param: `coef_tab` not supplied to fn, not saving it.')
  }

  if (!missing(cm)) {
    # Confusion Matrix Materials here.

    dir.create(file.path(path, 'Evaluation'), showWarnings = FALSE)

    utils::write.csv(
      #  save confusion matrix from old school split.
      data.frame(t(cm$table)),
      row.names = FALSE,
      file = file.path(path, 'Evaluation', paste0(taxon, '-CMatrix.csv'))
    )

    utils::write.csv(
      # save the calculated evaluation metrics.
      data.frame(
    ...
    message('param: `cm` not supplied to fn, not saving it.')
  }

  if (!missing(thresh)) {
    dir.create(file.path(path, 'Threshold'), showWarnings = FALSE)
    utils::write.csv(
      # threshold information - make it tidy.
      data.frame(
    ...
    message('param: `thresh` not supplied to fn, not saving it.')
  }

  if (!missing(f_rasts)) {
    dir.create(file.path(path, 'Raster'), showWarnings = FALSE)
    terra::writeRaster(
      # save all final rasters
      f_rasts,
      overwrite = TRUE,
      filename = file.path(path, 'Raster', paste0(taxon, '.tif'))
    )
  } else {
    message('param: `f_rasts` not supplied to fn, not saving it.')
  }
}
