...
#' @param method character passed onto NbClust when fixedClusters == TRUE.
#' @param min.nc = 3 Passed onto NbClust when fixedClusters == TRUE, minimum number of clusters to accept.
#'@param max.nc = 20 Passed onto NbClust when fixedClusters == TRUE, maximum number of clusters to accept.
...
EnvironmentalBasedSample <- function(
  ...
  weighted_mat <- extract_weighted_matrix(pred_rescale, f_rasts, lyr, n_pts)
  ...
  # Build raw -> geographic label lookup using the same stratified approach
  # as PosteriorCluster, then retrain the final KNN on geographic IDs so that
  # terra::predict in projectClusters returns IDs consistent with Geometry$ID.
  label_lookup <- reassign_cluster_ids(spatialClusters_planar, reordered$raster)
  geo_id_map   <- stats::setNames(label_lookup[, "new"], label_lookup[, "old"])

  weighted_mat$ID <- factor(
    unname(geo_id_map[as.character(weighted_mat$ID)])
  )
  weighted_mat <- weighted_mat[stats::complete.cases(weighted_mat$ID), ]

  finalKNN_geo    <- trainKNN(weighted_mat, split_prop = prop_split)
  final.fit       <- finalKNN_geo$fit.knn
  confusionMatrix <- final.fit$confusionMatrix

  # back to original crs for writeout
...
#' @param lyr Raster layer to select.
#' @return Data frame with extracted values, ready for clustering
#' @keywords internal
#' @noRd
extract_weighted_matrix <- function(pred_rescale, f_rasts, lyr, n_pts = 500) {
  pts <- terra::spatSample(
    f_rasts[[lyr]],
    as.points = TRUE,
    method = 'random',
...
write_cluster_results <- function(
  ...
  sf::st_write(
    ClusterVectors,
    file.path(path, 'ClusterVectors', paste0(taxon, '.gpkg')),
    append = FALSE,
    quiet = TRUE
  )
}
