...
#'   for "Assist-warm" method). Higher values in this column assumed warmer.
...
PolygonBasedSample <- function(
  ...
  if (missing(x) || missing(zones) || missing(zone_key)) {
    stop(
      "Must supply `x` (species range), `zones` (spatial data for Ecoregions or STZs), and `zone_key` (grouping column)."
    )
  }
  ...
  if (
    decrease_method %in% climate_methods || increase_method %in% climate_methods
  ) {
    if (decrease_method == "Assist-warm" || increase_method == "Assist-warm") {
      if (is.null(warmest_col) || !warmest_col %in% names(zones)) {
        stop(
          "Assist-warm requires valid `warmest_col` variable in `zones` data."
        )
      }
    }
    if (
      decrease_method == "Assist-drier" || increase_method == "Assist-drier"
    ) {
      if (is.null(precip_col) || !precip_col %in% names(zones)) {
        stop(
          "Assist-drier requires valid `precip_col` variable in `zones` data."
        )
      }
    }
  }
  ...
  zones_sub <- dplyr::mutate(
    zones_sub,
    poly_area = as.numeric(sf::st_area(geometry))
  ) # in m^2
...
allocate_increase <- function(
  ...
  if (
    !identical(class(zones_sub[[zone_key]]), class(zone_summary[[zone_key]]))
  ) {
  ...
  if (remainder > 0) {
    if (method %in% c("Largest", "Smallest")) {
      ...
      top_indices <- order(zone_summary$polygon_ct, decreasing = TRUE)[
        1:min(remainder, n_zones)
      ]
  ...
  for (i in seq_len(n_zones)) {
    ...
    poly_alloc <- base_poly +
      c(rep(1, remainder_poly), rep(0, n_polys - remainder_poly))
...
split_cols <- function(dat, y, sep = '-') {
  ...
  if (inherits(ob, 'list')) {
    # if unequal length information use this.

    df <- t(as.data.frame(do.call(
      cbind,
      lapply(ob, \(x) {
    ...
    df <- data.frame(t(ob))
  } # else transform straight to data frame

  df = stats::setNames(
    # convert from characters to numbers for math
    data.frame(apply(df, FUN = as.numeric, MARGIN = 2)),
    nm = c('lower', 'upper')
  )

  ## if the upper value is missing, make it the same as the lower.
  if (any(is.na(df['upper']))) {
    # impute the upper value - assume same as lower.
  ...
  data.frame(
    ## return object
    df,
    median = apply(df, MARGIN = 1, FUN = function(x) {
...
#' @keywords internal
#' @noRd
proportional_round <- function(
  values,
  target_sum,
  method = c("larger_up", "larger_down")
) {
