...
Baileys). The sampling of these products are all implemented using a
...
zones exist a **decrease** method needs to be implemented. Three options
are available for both scenarios. 1) Using the number of polygons per
class 2) using the total area of each class 3) considering simple
climate parameters for each class

Additionally, this function can be used on the cluster groups identified
using the Isolation by Distance, Resistance, and Environmental
